...
type ControlMapping struct {
	...
	ID       int    `json:"id"`
	Is3State bool   `json:"is_3state"` // true if 3-position switch (-1, 0, 1)
}
...
func (g *Game) processWizardText(input string) {
	...
	case StepSelectMode:
		...
		if g.settings.DeviceType == "radiomaster" {
			...
			g.outputLines = append(g.outputLines, fmt.Sprintf("Mode set to %s.", g.settings.Mode), "How many switches/buttons to map? (0-16):")
		}
	...
	case StepOtherNumButtons:
		...
		if g.otherBtnCount > 0 {
			...
			g.otherBtnIndex = 0
			g.snapshotBaselines()
			g.outputLines = append(g.outputLines, fmt.Sprintf("-> Trigger Switch/Button %d on controller...", g.otherBtnIndex+1))
		} else {
			g.wizardStep = StepOtherHasSlider
			g.outputLines = append(g.outputLines, "Do you have Slider 1? (y/n):")
		}
	...
	case StepOtherWaitButtonLabel:
		...
		g.tempPendingLabel = label
		g.wizardStep = StepOtherWaitButtonIs3State
		g.outputLines = append(g.outputLines, "Is this a 3-position switch (-1, 0, 1)? (y/n):")

	case StepOtherWaitButtonIs3State:
		is3 := strings.ToLower(input) == "y"
		g.settings.Mappings = append(g.settings.Mappings, ControlMapping{
			Name:     g.tempPendingLabel,
...
func (g *Game) processWizardPhysicalInput() {
	...
	case StepRMWaitSwitches:
		...
		if detectedBtn {
			...
			g.outputLines = append(g.outputLines, fmt.Sprintf("Mapped %s -> Btn %d (3-State: %v)", target, btnIdx, is3))
			g.advanceRMPrompt()
			return
		}
		...
		if detectedAxis {
			...
			g.outputLines = append(g.outputLines, fmt.Sprintf("Mapped %s -> Axis %d (3-State: %v)", target, axisIdx, is3))
			g.advanceRMPrompt()
			return
		}
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	// -------------------------------------------------------------------------
	// Top Right: Digital Buttons & Switches List
	// -------------------------------------------------------------------------
	startX := float32(360)
	...
	if g.configLoaded && len(g.settings.Mappings) > 0 {
		...
		for idx, m := range g.settings.Mappings {
			...
			if m.Is3State {
				// 3-Position Switch handling (-1, 0, 1)
				val := float32(0.0)
				if m.Type == "button" {
					if ebiten.IsGamepadButtonPressed(id, ebiten.GamepadButton(m.ID)) {
						val = 1.0
					}
				} else if m.Type == "axis" {
					val = getAxis(m.ID)
				...
				if val < -0.33 {
					// UP position (-1)
					btnColor = color.RGBA{0, 140, 200, 255}
					label = fmt.Sprintf("%s:UP(^)", m.Name)
				} else if val > 0.33 {
					// ON position (1)
					btnColor = color.RGBA{0, 200, 140, 255}
					label = fmt.Sprintf("%s:ON", m.Name)
				} else {
					// MID position (0)
					btnColor = color.RGBA{40, 44, 52, 255}
					label = fmt.Sprintf("%s:MID", m.Name)
				}
			} else {
				// Standard 2-state button / switch
				pressed := false
				if m.Type == "button" {
